...
class JacoController(Node):
    ...
    def find_rotation_pose(self, joy_msg:Joy):
        ...
        #target_position = self.get_frame(target_frame, world_frame)
        #reference_position = self.get_frame(aligning_frame, world_frame)
        roll = 0 
        pitch = 0
        target_frame = 'camera'
        aligning_frame = 'marker_5'
        glasses_from_marker = self.get_frame(target_frame, aligning_frame)
        if not (glasses_from_marker is None):
        ...
        orientation_x = (joy_msg.buttons[5] - joy_msg.buttons[4]) * MAX_ROTATION_CHANGE + (pitch * CORRECTION)
        orientation_y = (joy_msg.axes[3] * MAX_ROTATION_CHANGE)  
        orientation_z = (joy_msg.axes[4] * MAX_ROTATION_CHANGE) + (roll * CORRECTION)

        rot_target = Rotation.from_euler('xyz', [orientation_x, orientation_y, orientation_z], degrees=True)


        # Apply rotation to base
        quaternion_target = rot_base * rot_target  
