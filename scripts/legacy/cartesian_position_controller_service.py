...
class JacoController(Node):
    def __init__(self):
        ...
        self.current_pose = PoseStamped()  # Initialize current pose
        self.current_pose.header.frame_id = 'j2n6s300_link_base'
        self.current_pose.header.stamp = self.get_clock().now().to_msg()

        # Initialize pose goal
        self.current_pose.pose.position.x = 0.0
        self.current_pose.pose.position.y = 50.0
        self.current_pose.pose.position.z = 50.0
        # Forward quaternion
        ...
        #self.current_pose.header.seq = 0
    ...
    def joy_callback(self, joy_msg):
        try:
            ...
            self.current_pose.header.frame_id = 'j2n6s300_link_base'
            self.current_pose.header.stamp = self.get_clock().now().to_msg()
            #self.current_pose.header.seq += 1
            self.current_pose.pose = pose_goal

            # Send pose goal to action server
            self.pose_client.wait_for_server()
            self.get_logger().info(f'Pose goal is being sent: {self.current_pose}')
            self.pose_client.send_goal_async(ArmPose.Goal(pose=self.current_pose))
